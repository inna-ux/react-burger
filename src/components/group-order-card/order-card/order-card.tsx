    ...
    >
      <div className={`${styles.info} mt-6 mb-6`}>
        <p className={`${styles.number} text text_type_digits-default`}>
          #{ordersDataItem.number}
        </p>
        <p
          className={`${styles.date} text text_type_main-default text_color_inactive`}
        >
          <FormattedDate date={new Date(ordersDataItem.createdAt)} />
        </p>
      </div>

      <h4 className={`${styles.name} text text_type_main-medium mb-6`}>
        {ordersDataItem.name}
      </h4>
      ...
      <div className={`${styles.info} mb-6`}>
        <div className={`${styles.ingredients}`}>
          <ul className={`${styles.icon__ingredients}`}>
            ...
            {count ? (
              <div
                className={`${styles.card__count} text text_type_main-default`}
              >
                +{count}
              </div>
            ) : null}
